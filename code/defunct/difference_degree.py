...
# -*- coding: utf-8 -*-
"""
Created on Tue Aug  2 21:16:46 2022
...
df_degree=df.groupby(['year','Employer', 'Degree']).agg(
max_csr = pd.NamedAgg(column='main_dict', aggfunc=lambda x: np.max(x)),
mean_csr = pd.NamedAgg(column='main_dict', aggfunc=lambda x: np.mean(x)),
min_csr = pd.NamedAgg(column='main_dict', aggfunc=lambda x: np.min(x) ),
...
df_degree.to_csv('/global/home/pc_moseguera/data/Burning Glass 2/merged_variables/merge_main/2020/difference_deg.csv.zip', compression='zip', index=False)
