...
reg_1 <- felm(main_mean_str ~ as.factor(quartile_unemp) | as.factor(year) | 0 | Employer:BestFitMSA, data = panel_main)
reg_3 <- felm(main_mean_str ~ as.factor(quartile_unemp) + college + parttime + bon_com | as.factor(BestFitMSA) + as.factor(year):as.factor(SOC):as.factor(Sector) + IsSpecialized | 0 | Employer, data = panel_main)
reg_4 <- felm(main_mean_str ~ as.factor(quartile_unemp) + college + parttime + bon_com + logSize + gprofit + mlev + tonbiq + roe | as.factor(BestFitMSA) + as.factor(year):as.factor(SOC) + IsSpecialized + as.factor(Employer) | 0 | Employer, data = panel_main)
reg_5 <- felm(main_mean_str ~ as.factor(quartile_unemp) + college + parttime + bon_com | as.factor(BestFitMSA) + as.factor(SOC) + IsSpecialized + as.factor(Employer):as.factor(year) | 0 | Employer, data = panel_main)

texreg(list(reg_1, reg_3, reg_4, reg_5), digits = 4, file = '/global/home/pc_moseguera/data/Burning Glass 2/Regression Tables/firm_level/unemp_quartile_mean_1010.txt')
