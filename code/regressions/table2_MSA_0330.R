...
reg_5<-felm(log_sal ~ emp_ben_ind+below25_unemp | as.factor(year) | 0 | CanonEmployer, data=panel_main)
reg_6<-felm(log_sal ~ below25_unemp:emp_ben_ind+below25_unemp+emp_ben_ind+college+parttime+bon_com | as.factor(year):as.factor(OccFam)+as.factor(BestFitMSA)+IsSpecialized  | 0 | CanonEmployer, data=panel_main)
reg_7<-felm(log_sal ~ below25_unemp:emp_ben_ind+below25_unemp+emp_ben_ind+college+parttime+bon_com | as.factor(BestFitMSA)+as.factor(year):as.factor(OccFam)+as.factor(CanonEmployer)+IsSpecialized  | 0 | CanonEmployer, data=panel_main)
reg_8<-felm(log_sal ~ below25_unemp:emp_ben_ind+below25_unemp+emp_ben_ind+college+parttime+bon_com | as.factor(BestFitMSA)+as.factor(OccFam)+as.factor(CanonEmployer):as.factor(year)+IsSpecialized  | 0 | CanonEmployer, data=panel_main)


texreg(list(reg_1, reg_2, reg_3, reg_4, reg_5, reg_6, reg_7, reg_8), digits =4, file='/global/home/pc_moseguera/data/Burning Glass 2/Regression Tables/quarter/table2_MSA_0330.txt')
